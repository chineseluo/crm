...
<body>
	...
	<div class="button_bar">
		...
		<button class="common_button" onclick="to('<%=path%>/service/CstServiceAction_toDealCstServicePage');">返回</button>
	...
	<table class="query_form_table" id="table3">
		<tr>
			<th>分配给</th>
			<td>${model.svrDueTo }</td>
			<th>分配时间</th>
			<td><fmt:formatDate value="${model.svrDueDate }" pattern="yyyy年MM月dd日  HH时mm分ss秒" /></td>
		</tr>
	</table>
	<br />
	<form action="<%=path%>/service/CstServiceAction_dealCstServiceByManager" name="editFrom" method="post">
		<input type="hidden" name="id" value="${model.id}" />
		<table class="query_form_table" id="table1">
			<tr>
				<th>服务处理</th>
				<td colspan="3"><textarea name="svrDeal" rows="6" cols="50" tip="服务处理不能为空" reg="^.+$">${model.svrDeal}</textarea><span class="red_star">*</span></td>
			</tr>
			<tr>
				<th>处理人</th>
				<td><input type="hidden" name="svrDealerDTO.id" value="${sessionScope.userDto.id}" /> <input name="svrDealBy" value="${sessionScope.userDto.usrName}" readonly size="20" /><span class="red_star">*</span></td>
				<th>处理时间</th>
	...
	<table class="query_form_table" id="table2">
		<tr>
			<th>处理结果</th>
			<td><input disabled="disabled" name="svrResult" size="20" value="${model.svrResult }" /><span class="red_star">*</span></td>
			<th>满意度</th>
			<td><select name="svrSatisfy" disabled="disabled">
					<option>请选择...</option>
