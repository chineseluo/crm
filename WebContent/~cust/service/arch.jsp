...
<form name="searchForm" action="<%=path%>/service/CstServiceAction_toArchCstServicePage" method="post">
	...
	<table class="data_list_table">
		...
		<c:forEach items="${requestScope.pageResult.list }" var="service">
			<tr>
				...
				<td class="list_data_op"><img onclick="to('<%=path %>/service/CstServiceAction_toDetailArchCstServicePage?id=${service.id }');" title="查看" src="<%=path %>/images/bt_detail.gif" class="op_button" /></td>
			</tr>
		</c:forEach>
		<tr>
			<th colspan="100" class="pager">
				<form action="<%=path%>/service/CstServiceAction_toArchCstServicePage" method="post" name="pagingForm">
					<div class="pager">
